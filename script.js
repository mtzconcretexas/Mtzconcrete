...
function renderWhy() {
  $("whyEyebrow").textContent = t(CONFIG.why.eyebrow);
  $("whyTitle").textContent = t(CONFIG.why.title);
  $("whyLead").textContent = t(CONFIG.why.lead);
...
function initCounters() {
  ...
  const animate = el => {
    ...
    const duration = target >= 500 ? 1800 : 1300;
...
function initPremiumMotion() {
  const revealTargets = document.querySelectorAll(".services-heading, .service-card, .area-heading, .area-item, .projects-heading, .project-slider, .estimate-copy, .contact-details, .follow-work, .estimate-form, .stat-item, .reveal");
...
function renderLanguage() {
  ...
  renderNav(); renderHero(); renderServices(); renderWhy(); renderArea(); renderGallery();
}
