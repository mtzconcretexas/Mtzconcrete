...
function renderCompany() {
  ...
  phoneIds.forEach(id => {
    const el = $(id);
    if (!el) return;
    el.href = `tel:${CONFIG.company.phoneDial}`;
  });
  ...
  CONFIG.social.networks.forEach(network => {
    ...
    card.innerHTML = `
      ...
      <div class="social-icon">
        ${logo ? `<img class="social-brand-logo" src="./${logo}" alt="${network.name}">` : `${iconSvg(network.icon)}`}
      </div>
      <h3>${network.name}</h3>
      <p>${CONFIG.company.brandMain} ${CONFIG.company.brandSub}</p>
    `;
...
function renderEstimate() {
  ...
  document.querySelectorAll("[data-label]").forEach(el => {
    ...
    if (CONFIG.formLabels[key]) el.textContent = text(CONFIG.formLabels[key]);
  });
